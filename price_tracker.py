"""
Steam Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Steam.

Docs  : https://docs.brightdata.de/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.de/products/insights/price-tracker/steam
"""
...
# Set your Web Scraper ID after building your Steam scraper:
# https://brightdata.de/products/web-scraper/steam
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Build your Steam scraper at: https://brightdata.de/products/web-scraper/steam\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Steam product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Steam products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Steam category page.

    Example:
        results = discover_by_category("https://store.steampowered.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://store.steampowered.com/app/570/Dota_2/",
        # Add more Steam product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "steam_prices.json"
    ...
    # cat_results = discover_by_category("https://store.steampowered.com/category/sample", limit=50)
